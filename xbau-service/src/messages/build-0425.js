...
/**
 * build-0425: statistik.baufertigstellung.0425
 *
 * XSD: xbau-nachrichten-statistik.xsd
 * Rechtsgrundlage: § 1 (2) Nr. 2 HBauStatG
 * Statistikmeldung anlaesslich der Baufertigstellung.
 *
 * Pflichtfelder: bezug (BezugStatistikmeldung), allgemeineAngaben,
 *   datenBaufertigstellung (DatenBaufertigstellung), datenEinzelnesGebaeude
 * Optional: ansprechpartner
 */
export function buildStatistik0425(params) {
  ...
  const root = doc
    .ele(NS_XBAU, "statistik.baufertigstellung.0425")
  ...
  const dbf = root.ele(NS_XBAU, "datenBaufertigstellung");
  dbf.ele(NS_XBAU, "datumBaufertigstellung").txt(params.datenBaufertigstellung.datumBaufertigstellung);
  if (params.datenBaufertigstellung.aktenbereinigungBaufertigstellung) {
    dbf.ele(NS_XBAU, "aktenbereinigungBaufertigstellung").txt("true");
  }
