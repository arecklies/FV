...
**Letzte Verfeinerung:** 2026-03-26 (req-refine: ADR-008 referenziert, Abbruch-AC ergaenzt, Export-Sicherheit auf authentifizierte API-Route gesetzt)
...
- **ADR-008:** Asynchrone Verarbeitung -- Export als Background Job (Edge Function, Level 2)
...
- FA-8: Export-Download nur ueber authentifizierte API-Route (kein separater Storage-Link)

## 4. User Stories & Akzeptanzkriterien

### US-1: Mandanten-Export
Als Amtsleiter moechte ich jederzeit alle Daten meiner Behoerde exportieren.
- AC-1: Export-Button im Admin-Bereich (nur Rolle >= `amtsleiter`, ADR-002)
...
### US-2: Export-Performance und Steuerung
Als Admin moechte ich den Export in akzeptabler Zeit abschliessen und bei Bedarf abbrechen.
- AC-1: Export von 10.000 Vorgaengen in < 4 Stunden
- AC-2: Fortschrittsanzeige waehrend Export (aktueller Schritt, Prozent)
- AC-3: Export laeuft asynchron (Background Job via ADR-008), Benachrichtigung bei Fertigstellung
- AC-4: Laufender Export kann abgebrochen werden (Job-Status auf "cancelled" setzen)
- AC-5: Download nur ueber authentifizierte API-Route (keine Signed URL, keine externe Zugriffsmoeglickeit)
...
- NFR-4: Asynchrone Verarbeitung (nicht synchron in API-Route - Vercel 55s Limit, ADR-008)
- NFR-5: Export-Download nur ueber authentifizierte API-Route -- keine externen Links, keine Signed URLs

## 6. Spezialisten-Trigger

- **Backend Developer:** ExportService, asynchrone Verarbeitung (ADR-008), ZIP-Erzeugung
- **Security Engineer:** Tenant-Isolation im Export, keine Cross-Tenant-Daten, Download-Absicherung
...
2. ~~Dokumenten-Export: Original-Dateien oder nur Metadaten?~~ **Geklaert:** Original-Dateien (Annahme bestaetigt).
3. ~~Wie wird der asynchrone Export technisch umgesetzt?~~ **Geklaert:** ADR-008 Level 2: Edge Function, Chunked, background_jobs-Tabelle.
4. ~~Export-Sicherheit?~~ **Geklaert:** Nur Download ueber authentifizierte API-Route, kein separater Storage-Link.

## 8. Annahmen

- Export laeuft asynchron (Background Job via ADR-008, Edge Function Level 2)
- ZIP-Erzeugung serverseitig
- Original-Dokumente werden als Dateien exportiert (nicht Base64 in JSON)
- Download nur ueber authentifizierte API-Route (Session-Cookie geprueft)
...
| ADR-003 (Service-Architektur) | ExportService |
| ADR-008 (Background Jobs) | Export als Edge Function (Level 2), background_jobs-Tabelle |
...
| Export bei grossen Tenants zu langsam | Mittel | Timeout, Abbruch | Asynchrone Verarbeitung, Chunking (ADR-008) |
